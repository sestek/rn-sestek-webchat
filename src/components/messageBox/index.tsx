...
const MessageBox: FC<PropsMessageBoxComponent> = (props) => {
  ...
  const renderItemMessage = () => {
    return (
      <>
        ...
        {props.type === 'message' &&
          props.activity?.attachments && props.activity?.attachments[0]?.content?.title && (
            <Markdown styles={styles.rceMboxText}>
              {props.activity?.attachments[0]?.content?.title}
            </Markdown>
          )}
        {props.type === 'message' &&
          props.activity?.attachments && props.activity?.attachments[0]?.content?.subtitle && (
            <Markdown styles={styles.rceMboxText}>
        ...
        {props.activity?.type === 'message' &&
          props.activity?.attachments && props.activity?.attachments[0]?.content?.text && (
        ...
        {Array.isArray(props.activity?.attachments) &&
          props.activity?.attachments &&  props.activity?.attachments[0]?.content?.buttons?.map(
  ...
  const renderTyping = () => {
    return (
      ...
      // <Lottie
      //   source={props.customizeConfiguration.typing}
      //   speed={0.7}
      //   autoPlay
      //   loop
      //   style={{
      ...
      {props.type === 'system' ? null : (
        <View style={[positionCls]}>
          <View style={styles.rceMboxBody}>
            ...
            {props.position === "right" ? renderTyping(): null}
